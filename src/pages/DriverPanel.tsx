...
export default function DriverPanel() {
  ...
  const fetchTrips = () => api.get("/trips/driver").then(res => setTrips(res.data));
  const fetchNotifications = () => api.get("/notifications/driver").then(res => setNotifications(res.data));
  ...
  return (
    <div className="p-4 grid grid-cols-2 gap-6">
      <div>
        <h1 className="text-2xl font-bold mb-4">Статистика поездок</h1>
        ...
        <ul className="list-disc list-inside">
          {chartData.map((c, idx) => (
            <li key={idx}>{c.trip}: {c.earned} тг</li>
          ))}
        </ul>
      </div>
